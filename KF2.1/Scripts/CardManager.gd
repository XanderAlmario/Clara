...
func stopDragging():
	...
	if draggingCard.cost <= BMRef.playerDev:
		if draggingCard.cardType != "Unit":
			for card in $"../BattleManager".playerBF.unitsInPlay:
				if card.momentum:
					card.attack += 1
					card.health += 1
					card.get_node("Attack").text = str(card.attack)
					#card.get_node("Inspect/Attack").text = str(card.attack)
					card.get_node("Health").text = str(card.health)
					#card.get_node("Inspect/Health").text = str(card.health)
			
			if draggingCard.name == "Frozen Spike":
				if BMRef.enemyUnitsOnBF.size() == 0:
