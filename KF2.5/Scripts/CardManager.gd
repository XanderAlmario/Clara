...
func play_card_here_and_for_clients_opponent(player_id, card_name, card_slot_name):
	...
	if card:
		...
		if card.cardName == "Training Mentor":
			$"../BattleManager".spawn_token("Monk")
		
		if card.cardName == "Way of Mercy Master":
			createChai()
		
		if card.cardName == "Way of Open Hand Master":
			for unit in $"../BattleManager".playerBF.unitsInPlay:
				if !unit.fury:
					unit.fury = true
		
		for unit in $"../BattleManager".playerBF.unitsInPlay:
			if unit.cardName == "Way of Open Hand Master":
				card.fury = true
