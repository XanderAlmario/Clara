		...
		if draggingCard.cardType != "Unit":
			if draggingCard.spellScript:
				if draggingCard.spellScript.has_method("trigger_targeted_ability"):
					var target = null
					if draggingCard.name == "Astral Projection":
						target = checkEnemyTarget()
						if not target:
							target = checkCard() 
					elif draggingCard.name == "Cup of Chai":
						target = checkCard() 
					else:
						target = checkEnemyTarget()
						
					if target:
						var was_successful = draggingCard.spellScript.trigger_targeted_ability(target, BMRef, true)
						if was_successful:
					...
					var was_successful = draggingCard.spellScript.trigger_ability(deckRef, BMRef)
...
@rpc("any_peer")
func sync_astral_projection(target_name: String, new_copy_name: String):
	var opponent_field = get_parent().get_parent().get_node("EnemyField")
	var player_field = get_parent().get_parent().get_node("PlayerField")
	
	var original = get_node_or_null(target_name)
	if not original:
		original = opponent_field.get_node_or_null("CardManager/" + target_name)

	if original:
		var copy = original.duplicate()
		copy.name = new_copy_name
		
		var enemy_bf = player_field.get_node("EnemyBattlefield")
		var enemy_cm = player_field.get_node("CardManager")
		
		enemy_cm.add_child(copy)
		enemy_bf.unitsInPlay.append(copy)
		copy.cardSlot = enemy_bf
		...
		updateCardsOnBF(enemy_bf)
		copy.get_node("AnimationPlayer").play("cardFlip")
